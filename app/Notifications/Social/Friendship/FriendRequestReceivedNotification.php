...
class FriendRequestReceivedNotification extends Notification implements ShouldBroadcast, ShouldQueue
{
    use Queueable;
    ...
    {
        $userUrl = str_replace('/api', '', route('users.show', ['identifier' => $this->sender->username]));

        return [
            'message' => "Ви отримали запит у друзі від <a href=\"{$userUrl}\">{$this->sender->username}</a>",
